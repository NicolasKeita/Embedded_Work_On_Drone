/*
Filename: Src/Runners/MonteCarlo/Export/MonteCarloCampaign-Json.cpp
Description: JSON campaign report writer associating perturbed run inputs with measured output metrics.
...
namespace {
    ...
    void write_json_record(std::ostream& out, const RunRecord& record, bool last)
    {
        ...
        out << "    \"status\": \"" << (record.metrics.passed ? "PASS" : "FAIL") << "\",\n";
        out << "    \"inputs\": {\n";
