...
constexpr double kTimeEpsilon = 1.0e-9, kDegreesPerRadian = 180.0 / std::numbers::pi;
                                       ...
                                       double                telemetry_report_interval_s)
{
    const double fault_time = record.result.fault_injected_time;
        ...
        out << "\n#### Telemetrie mission\n\n";
    write_telemetry_header(out);
    const double pre_fault_stop = fault_time < 0.0 ? std::numeric_limits<double>::max() : fault_time;
    ...
    write_telemetry_header(out);
    write_telemetry_rows(out, record.telemetry, fault_time, std::numeric_limits<double>::max(),
