/*
Filename: Tests/Scenarios/FlightScenarios.cpp
...
void autonomous_altitude(TestHarness& runner, double hover_rpm)
{
    ...
    runner.check(trace.metrics.time_within_tolerance > 0.0,
                 "G1 : altitude dans la tolerance (+/- 2 m)");
    runner.check(trace.metrics.overshoot_percent() < 10.0,
                 "G2 : depassement contenu (< 10 % de l'ecart initial)");
    runner.check(trace.metrics.steady_state_error < 1.0,
                 "G3 : erreur residuelle faible (< 1 m)");
...
void autonomous_position_x(TestHarness& runner, double hover_rpm)
{
    ...
    run_mission(controller, aircraft,
                {.target = {.x = 20.0, .z = 100.0}, .duration = 180.0,
                 .axis = TrackingAxis::x_axis, .tolerance = 1.0, .stop_on_zone = true});

    std::cout << "-- Phase 2 : retour vers x = 0 --" << std::endl;
    const MissionRunTrace trace =
        run_mission(controller, aircraft,
                    {.target = {.z = 100.0}, .duration = 120.0,
                     .axis = TrackingAxis::x_axis, .tolerance = 0.5});

    print_metrics_report("axe X", trace.metrics);
    runner.check(trace.metrics.time_within_tolerance > 0.0,
                 "H1 : consigne x = 0 atteinte (+/- 0,5 m)");
    runner.check(trace.metrics.overshoot_percent() < 15.0,
                 "H2 : depassement contenu (< 15 % de l'ecart initial)");
    runner.check(trace.metrics.steady_state_error < 0.5,
                 "H3 : erreur residuelle faible (< 0,5 m)");
}
...
void autonomous_position_y(TestHarness& runner, double hover_rpm)
{
    ...
    run_mission(controller, aircraft,
                {.target = {.y = -15.0, .z = 100.0}, .duration = 180.0,
    ...
    const MissionRunTrace trace =
        run_mission(controller, aircraft,
                    {.target = {.z = 100.0}, .duration = 120.0,
                     .axis = TrackingAxis::y_axis, .tolerance = 0.5});

    print_metrics_report("axe Y", trace.metrics);
    runner.check(trace.metrics.time_within_tolerance > 0.0,
                 "I1 : consigne y = 0 atteinte (+/- 0,5 m)");
    runner.check(trace.metrics.overshoot_percent() < 15.0,
                 "I2 : depassement contenu (< 15 % de l'ecart initial)");
    runner.check(trace.metrics.steady_state_error < 0.5,
                 "I3 : erreur residuelle faible (< 0,5 m)");
}
