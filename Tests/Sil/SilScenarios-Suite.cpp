...
Description: SIL FC1-failure-during-climb scenario (FAULT_INJECTOR-005) and the suite
...
void fc1_failure_during_climb_scenario(TestHarness& runner, SilRunOutput& output, ScenarioRecord& record)
{
    ...
    if (!outcome.has_value()) {
        ...
        output = SilRunOutput{};
        record = {.name = "FAULT_INJECTOR-005", .scenario = scenario, .result = SimulationResult{}};
        return;
    }
    ...
    record = {.name = "FAULT_INJECTOR-005", .scenario = scenario, .result = r, .events = output.events,
...
const std::array<SilScenarioEntry, 6> kSilScenarios{{
    ...
    {"FAULT_INJECTOR-005",  "FC1 failure during the climb transition",   fc1_failure_during_climb_scenario},
}};
