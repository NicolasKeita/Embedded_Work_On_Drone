...
| :--- | :--- | :--- |
| `flight.hal.types` | `hal/hal_types.cppm` | Structures HAL `SensorData` / `ActuatorCommands` et masques de validité `kFlagImu1Ok` ... `kFlagTachometerOk` |
| `flight.hal.sensor` | `hal/sensor_input.cppm` | Interface abstraite `ISensorInput` (acquisition capteurs, non bloquante) |
| `flight.hal.actuator` | `hal/actuator_output.cppm` | Interface abstraite `IActuatorOutput` (émission des commandes) |
| `flight.hal.clock` | `hal/clock.cppm` | Interface abstraite `IClock` (temps monotone en microsecondes) |
| `flight.transport` | `transport/transport.cppm` | Interfaces `ITransport` (canal d'octets full-duplex) et `ITransportStream` (vue flux brut, réservée au driver UART STM32) |
| `flight.transport.protocol` | `transport/hil_protocol.cppm` | Contrat filaire HIL-Proto v1.0 : constantes, `HilHeader`, payloads `#pragma pack(1)`, `ActuatorDiagnostics`, `static_assert` de tailles |
| `flight.transport.protocol.parser` | `transport/parser/hil_protocol_parser.cppm` + `-crc.cpp`, `-sync.cpp`, `-frame.cpp` | Réception : `HilCrc` (CRC-16-CCITT) et `HilFrameParser` (FSM octet par octet, zéro allocation) |
| `flight.transport.protocol.codec` | `transport/codec/hil_protocol_codec.cppm` + `-sensor.cpp`, `-actuator.cpp` | Conversion HAL <-> payload wire et sérialisation de trames complètes (Header + Payload + CRC) |
| `flight.sim.clock` | `sim/sim_clock.cppm` | `SimulatedClock` : horloge virtuelle avancée par le driver lockstep (PC) |
| `flight.sim.loopback` | `sim/loopback_transport.cppm` + `.cpp` | `LoopbackTransport` : ring buffer fixe de 512 octets simulant le lien série |
| `flight.sim.sensor` | `sim/sim_sensor_input.cppm` + `.cpp` | `SimulatedSensorInput` : injecte un échantillon simulé, le FC le lit via `ISensorInput` |
| `flight.sim.actuator` | `sim/sim_actuator_output.cppm` + `.cpp` | `SimulatedActuatorOutput` : enregistre la dernière commande émise par le FC |
| `flight.demo` | `demo/demo.cppm` + `demo-Sensor.cpp`, `demo-Actuator.cpp`, `demo-Step.cpp`, `demo-Report.cpp` | Démonstration de fermeture de boucle d'un pas lockstep complet |
...
Les constantes et vérifications de compilation proviennent de `flight.transport.protocol` :
...
Le simulateur PC hôte n'a besoin que des modules `flight.sim.*`, `flight.transport.*` et `flight.hal.types`. Squelette de boucle lockstep 100 Hz :
...
* **Contrat compilé** : les `static_assert` de `flight.transport.protocol` empêchent toute divergence silencieuse entre la doc et le code.
...
1. Déclarer la constante `kMsgIdTimeSync = 0x03` et la structure `#pragma pack(1)` correspondante dans `flight.transport.protocol`, avec son `static_assert` de taille.
