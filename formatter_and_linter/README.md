...
- **Module size** : `[WARN_MODULE_TOO_LARGE] Le module/namespace '<Name>' compte N fichiers d'implémentation (seuil : 8). Pense à le subdiviser en sous-modules.` Implementation files (.cpp) are grouped by filename functional prefix (ex: `SilScenarios-Observability-*`) or C++20 module declaration (`module <Name>;`); when a group exceeds 8 files a warning suggests splitting it into sub-modules
